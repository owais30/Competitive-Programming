...
int main() {
  ...
  while (1) {
    fgets(line, 100, stdin);  //fgets(pointer to str, max_num_including_end_of_line_char, stream)
    line[(int)strlen(line) - 2] = 0; // delete dummy char
    if (strncmp(line, ".......", 7) == 0) break;
    //int strncmp ( const char * str1, const char * str2, size_t num );
    //returns <0, =0, >0 according to match
  ...
  for (i = digits = alphas = vowels = consonants = 0; str[i]; i++) { // we can use str[i] as terminating condition as string in C++ is also terminated with NULL (0)
    str[i] = tolower(str[i]); // make each character lower case defined in ctype.h
  ...
  int hascs3233 = (strstr(str, "cs3233") != NULL);//strstr returns pointer to substring only if it is found else returns null pointer
